...
struct dinic
{
    ...
    vector<edge> get_mincut(int s, int t) {
        vector<edge> mincut;
        bfs(s, t); // Llama a BFS para establecer los niveles correctos

        for (int u = 0; u < n; ++u) {
            if (level[u] < n) {
                for (edge &e : adj[u]) {
                    if (level[e.dst] == n && e.cap - e.flow > 0) {
                        mincut.push_back(e); // Aristas que van de un nodo visitable a uno no visitable
                    }
                }
            }
        }
